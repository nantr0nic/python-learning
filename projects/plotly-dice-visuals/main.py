...
def main():
    print("===== Plotly Roll Data HTML Generator Software Program Utility App =====")
    ...
    title = f"Results of rolling {dice_string} {num_rolls} times."
    labels = {"x": "Result", "y": "Frequency of Result"}
    fig = px.bar(x=poss_results, y=freq_data, title=title, labels=labels)

    while True:
        choice = (
            input("Do you want to (d)isplay or (s)ave the file? (S/D) >> ")
            .lower()
            .strip()
        )
        match choice:
            ...
            case "s":
                ...
                if valid_filename(filename):
                    ...
                    print(f"Your results have been rendered to: {filename}.html")
                    break
                else:
                    print("Please enter a valid filename!")
                    continue
            case _:
                print("Please choose (D)isplay or (S)ave!")
                continue
